...
// MariaDB devuelve DATETIME como Date en UTC.
// Ecuador = UTC-5, sumamos 5h para recuperar hora local.
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      alternativas: {
        antes:   alternativas.filter(a => a.direccion === 'antes').map(a => a.fecha_hora).reverse(),
        despues: alternativas.filter(a => a.direccion === 'despues').map(a => a.fecha_hora)
      }
    });
...
router.get('/slots', async (req, res) => {
  ...
  try {
    ...
    while (totalMin < totalFin) {
      ...
      const [check] = await conn.query('SELECT fn_slot_disponible(?) AS disponible', [fechaHora]);
      slots.push({ hora: `${hh}:${mm}`, fecha_hora: fechaHora, disponible: check[0].disponible === 1 });
